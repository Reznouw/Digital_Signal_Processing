...
xr.fijobsnr1 = snr_bloque(x,xr.fijo1,T_V,Fs); %rp1
...
r=24;
Tc=6;%tiempo de captura 
Nc=1;%mono


%cOMANDO PARA RECONOCER PUERTOS DE ENTRADA Y SALIDA DE AUDIO DE LA
%COMPUTADORA 
A = audiodevinfo();
ID_Imput = 1; 
ID_Output = 3; 
%Se crea un objeto de grabacion 
grabadora = audiorecorder(Fs,nBits, Nc);
...
subplot(3,1,1);
plot(t,x,"LineWidth",2);
xlabel("Tiempo(s)");ylabel("Amplitud(V)"),title("Señal en el timpo ");
...
%Reproducir de la señal
%Creo un objetivo en produccion, una reproductora 
reproductor = audioplayer(x,Fs/2,r,ID_Output);
play(reproductor)

%Trnaformada de fourier
[X,FREC] = fourier(X,Fs);
